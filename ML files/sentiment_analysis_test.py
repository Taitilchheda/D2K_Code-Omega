...
# --- Sentiment Analysis on Text Column --- #
# Let's simulate a text column with random reviews or comments (for demonstration purposes)
# In practice, you would replace this with actual text data (e.g., user reviews)

ticket_sales_data['text'] = ticket_sales_data['sentiment_score'].apply(
    lambda x: f"Great event, highly recommended!" if x > 0.5 else 
              f"Could have been better, not recommended." if x < -0.5 else 
              f"Event was okay, nothing special.")
...
def apply_vader_sentiment(text):
    ...
    return sentiment['compound']  # Compound score represents the overall sentiment

ticket_sales_data['vader_sentiment'] = ticket_sales_data['text'].apply(apply_vader_sentiment)

# VADER sentiment analysis interpretation (optional)
ticket_sales_data['vader_analysis'] = ticket_sales_data['vader_sentiment'].apply(
    lambda x: 'Positive' if x > 0 else ('Negative' if x < 0 else 'Neutral'))

# Save the data to a CSV file
ticket_sales_data.to_csv('ticket_sales_data_with_vader_sentiment.csv')

print("Data saved to 'ticket_sales_data_with_vader_sentiment.csv'.")
